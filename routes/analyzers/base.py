...
async def run_analysis(analysis_id: str, data: dict, prompt_template: str):
    print(f"🔥🔥🔥 RUN_ANALYSIS STARTED for {analysis_id}")
    print(f"📦 Data received: {list(data.keys())}")
    print(f"📝 Prompt preview: {prompt_template[:100]}...")
...
This is a placeholder response. Add your DeepSeek API key to .env for real analysis."""
        ...
        if data["status"] == "complete":
            user_email = request.session.get("user_email")  # Need to pass this
            if user_email:
                before = get_user_tokens(user_email)
                deduct_token(user_email)
                after = get_user_tokens(user_email)
                print(f"💰 Token deduction for {user_email}: {before} → {after}")
        ...
        save_analysis(analysis_id, data)
        print(f"✅✅✅ RUN_ANALYSIS COMPLETED for {analysis_id}")
